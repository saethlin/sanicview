...
SV_Histogram::SV_Histogram(SV_Window* window) : SV_Widget(window) {
    this->x = 0;
    this->y = window->get_height()-50;
...
void SV_Histogram::draw() {
    ...
    if (this->histogram.size() == 0) {
        ...
        if (scaled.width() != width) {
            scaled = histogram.get_resize(width, height, 1, 3);
            ...
            scaled.draw_line(black_pos, 0, black_pos, height, red);
            scaled.draw_line(white_pos, 0, white_pos, height, red);

        } else {
            // Revert sliders and re-draw in new locations
            if (new_black_pos != black_pos) {
                scaled.column(black_pos) = black_column;
                scaled.draw_line(black_pos, y+height, black_pos, y, red);
                black_column = scaled.get_column(black_pos);
                black_pos = new_black_pos;
            }
            if (new_white_pos != white_pos) {
                scaled.column(white_pos) = white_column;
                scaled.draw_line(white_pos, y+height, white_pos, y, red);
                white_column = scaled.get_column(white_pos);
